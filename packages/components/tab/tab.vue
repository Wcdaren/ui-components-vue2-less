<template>
  <div class="r-tab-part">
    <div class="r-tab" :class="{ 'r-tab-horizontal': positionNavCss }">
      <div
      ...
      <div :class="titleClass" ref="navlist" :style="customHeight">
        ...
        <span
          ...
          :class="[
            titleNavList,
            'r-title-nav',
            { 'r-tab-disable': value.disable },
            { 'r-tab-active': activeIndex == index },
          ]"
        >
          <a
            class="r-tab-link"
...
export default {
  ...
  computed: {
    ...
    titleClass: function () {
      const smooth = this.smoothFlag ? 'r-tab-title-smooth' : ''
      if (this.positionNav == 'top') {
        return 'r-tab-title' + ' ' + smooth
      }
      return 'r-tab-title-' + this.positionNav + 'nav' + ' ' + smooth
    },
    ...
    titleNavList: function () {
      if (this.positionNav == 'top' || this.positionNav == 'bottom') {
        if (this.isScroll) {
          return 'r-title-nav-scroll'
        }
        return 'r-title-nav-list'
      } else {
        if (this.isScroll) {
          return 'r-title-vertical-scroll'
        }
        return 'r-title-nav-' + this.positionNav + 'nav'
      }
    },
  ...
  methods: {
    ...
    initTab: function (slot) {
      ...
      this.$nextTick(() => {
        ...
        if (this.positionNav == 'top' || this.positionNav == 'bottom') {
          this.navWidth =
            this.$refs.navlist.querySelector('.r-title-nav')?.offsetWidth
          tapWidth = this.$refs.navlist.offsetWidth
        } else {
          this.navWidth =
            this.$refs.navlist.querySelector('.r-title-nav')?.offsetHeight
